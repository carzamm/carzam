...
# Max File Size is 16MB Before Upload Is Aborted
...
def home_page():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            # Kevin - Note: temporarily commented out
            # getrecentfile = max(glob.glob(OUT_DIRECTORY + '*.jpg'), key=os.path.getctime)
            # print(f'getrecentfile = {getrecentfile}')
            # return render_template('index.html', filename = getrecentfile)

            # Kevin update - temporary return to unedited file while george edits yolo code
            return render_template('index.html', filename = IN_DIRECTORY + filename)
